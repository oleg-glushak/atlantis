...
func (g *GithubClient) HidePrevCommandComments(logger logging.SimpleLogging, repo models.Repo, pullNum int, command string, dir string) error {
	...
	for {
		...
		for _, comment := range query.Repository.PullRequest.Comments.Nodes {
			...
			if comment.Author.Login != "" && !strings.EqualFold(string(comment.Author.Login+"[bot]"), g.user) {
				logger.Debug("Skipping non matching author commentID %s[bot], %s, %s, %s", comment.Author.Login, comment.IsMinimized, comment.ID, g.user)
				continue
			}

			body := strings.Split(string(comment.BodyText), "\n")
			if len(body) == 0 {
				logger.Debug("Skipping empty body commentID %s, %s, %s, %s", comment.Author.Login, comment.IsMinimized, comment.ID, g.user)
				continue
			}
			firstLine := strings.ToLower(body[0])
			if !strings.Contains(firstLine, strings.ToLower(command)) {
				logger.Debug("Skipping missing command %s, %s, %s, %s", comment.Author.Login, comment.IsMinimized, comment.ID, g.user)
				continue
			}
			...
			if dir != "" && !strings.Contains(firstLine, strings.ToLower(dir)) {
				logger.Debug("Skipping dir whatever %s, %s, %s, %s", comment.Author.Login, comment.IsMinimized, comment.ID, g.user)
				continue
			}
	...
	for _, commentID := range allMatchingCommentIDs {
		...
		if err := g.v4Client.Mutate(g.ctx, &m, input, nil); err != nil {
			return errors.Wrapf(err, "minimize commentID %s", commentID)
		}
	}

	//for _, commentID := range allComments {
	//	// Crude filtering: The commentID templates typically include the command name
	...
	//	body := strings.Split(commentID.GetBody(), "\n")
	//	if len(body) == 0 {
	//		continue
	//	}
	//	firstLine := strings.ToLower(body[0])
